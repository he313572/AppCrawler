package com.honpe.crawler.technology;
...
@Component
public class ZolTechnologylCrawler implements PageProcessor {
	...
	public void process(Page page) {
		if (!page.getUrl().regex("http://geek.zol.com.cn/\\d{3}/\\d+.html").match()) {
			page.addTargetRequests(page.getHtml().css("div .TabBox").links().regex("http://geek.zol.com.cn/\\d{3}/\\d+.html").all());
		} else {
			String title = page.getHtml().xpath("//div[@class=article-header]/h1/text()").get();
			if (title != null) {
				int result = newsDao.findOneByTitle(title,TableType.TECHNOLOGY.getTable());
				if (result > 0)
					return;
				...
				try {
					newsDao.insertOne(news,TableType.TECHNOLOGY.getTable());
